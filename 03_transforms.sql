...
WITH sessions_daily AS (
  SELECT
    ...
    ROUND(SAFE_DIVIDE(COUNTIF(converted), COUNT(*)), 4) AS cvr,
...
SELECT
  ...
  ROUND(SAFE_DIVIDE(
    COALESCE(o.revenue_usd, 0), NULLIF(COALESCE(sp.spend_usd, 0), 0)
  ), 2)                             AS roas,
  ROUND(SAFE_DIVIDE(
    COALESCE(sp.spend_usd, 0), NULLIF(COALESCE(o.orders, 0), 0)
...
-- CVR lift and implied price elasticity by channel x discount tier.
  ...
  SELECT
    *,
    FIRST_VALUE(cvr) OVER (
      PARTITION BY channel ORDER BY discount_pct
    ) AS baseline_cvr
...
SELECT
  ...
  cvr,
  baseline_cvr,
  ROUND(SAFE_DIVIDE(cvr - baseline_cvr, NULLIF(baseline_cvr, 0)), 4) AS cvr_lift_pct,
  ROUND(SAFE_DIVIDE(
    SAFE_DIVIDE(cvr - baseline_cvr, NULLIF(baseline_cvr, 0)),
    NULLIF(-discount_pct, 0)
  ), 2)                          AS implied_elasticity
FROM with_baseline
WHERE sessions >= 50
...
-- One row per campaign — ROAS, ROI, revenue, and attributed orders.
...
SELECT
  ...
  ROUND(SAFE_DIVIDE(
    COALESCE(co.gross_margin_usd, 0) - c.budget_usd, NULLIF(c.budget_usd, 0)
  ), 2)                                            AS roi,
  ROUND(SAFE_DIVIDE(
...
-- Revenue and margin by product x discount tier.
...
SELECT
  ...
  ROUND(SAFE_DIVIDE(SUM(gross_margin_usd), NULLIF(SUM(revenue_usd), 0)), 4) AS margin_rate
FROM `YOUR_PROJECT.promo_roi.orders`
GROUP BY 1, 2, 3
ORDER BY product, discount_tier;


-- ── Ad-hoc: country revenue breakdown ────────────────────────────────────────
-- Run interactively in the BigQuery console — not a view.
